...
import { ArrowLeft, Shield, Sparkles, CheckCircle2, AlertTriangle } from 'lucide-react';
  ...
  if (!caseStudy) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4 text-center">
        <p className="text-sm uppercase tracking-wide text-primary-500 mb-2">Case Study</p>
        <h1 className="text-3xl font-bold text-gray-900 mb-4">We couldn’t find that story</h1>
        <p className="text-gray-600 mb-6 max-w-md">
          The case study you’re looking for may have moved or does not exist. Head back to see the full library.
        </p>
        <button
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
      <button
        ...
        <section className="bg-white border border-gray-100 rounded-3xl shadow-xl p-6 sm:p-10 mb-10">
          ...
          <p className="text-gray-600 text-base sm:text-lg mb-6">{caseStudy.overview}</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {caseStudy.metrics.map(metric => (
              <div key={metric.label} className="rounded-2xl border border-gray-100 bg-gray-50 px-4 py-5 text-center">
                <div className="text-2xl font-bold text-gray-900">{metric.value}</div>
                <div className="text-xs uppercase tracking-wide text-gray-500">{metric.label}</div>
                {metric.context && <div className="text-xs text-gray-400 mt-1">{metric.context}</div>}
              </div>
            ))}
          </div>
        </section>
        ...
        {caseStudy.quote && (
          <section className="bg-white border border-gray-100 rounded-3xl shadow-lg p-8 mb-12">
            <p className="text-xl text-gray-900 font-semibold mb-4">“{caseStudy.quote.text}”</p>
