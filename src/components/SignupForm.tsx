  ...
  const handleSocialSignup = async (provider: OAuthProvider) => {
    try {
      ...
      if (!isOAuthProviderConfigured(provider)) {
        setErrors({
          general: `${provider === 'google' ? 'Google' : 'GitHub'} authentication is not configured. Please contact support or use email signup.`,
        });
      ...
      // Note: User will be redirected, so we don't need to reset loading state
    } catch (error) {
      setOauthLoading(null);
      const errorMessage =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 409) {
          ...
          setErrors({
            general:
              data.message ||
              'Some of these details don’t look right. Give them a quick pass and try again.',
          });
        } else if (response.status >= 500) {
          setErrors({
            general:
              'Our signup service is under heavy load. Please wait a moment and try again.',
          });
        } else {
          setErrors({
            general:
      ...
      if (error instanceof TypeError && error.message.includes('fetch')) {
        setErrors({
          general:
            'Network issue detected. Check your connection and retry the signup.',
        });
      } else {
        setErrors({
          general:
    ...
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-white via-primary-50 to-primary-100">
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 py-12 sm:px-8 lg:py-16">
        <div className="grid gap-10 lg:grid-cols-2 items-center">
          {/* Left panel: value prop (match Login layout) */}
          <motion.div
          ...
          >
            <div className="inline-flex items-center gap-2 rounded-full bg-primary-100/70 px-3 py-1 text-xs font-semibold text-primary-700 shadow-sm border border-primary-200/70">
              ...
              Start safer AI-powered refactors
            </div>
            <div className="space-y-3">
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 leading-tight">
                Create your Refactron account
              </h1>
              <p className="text-base sm:text-lg text-gray-600">
                Join engineering teams using Refactron to modernize legacy
                services, reduce review time, and keep every refactor
                reviewable.
              </p>
            </div>
          ...
          {/* Right panel: auth card (mirrors Login layout) */}
          <motion.div
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
                ...
                >
                  ...
                  <div className="mt-3 space-y-1.5 text-[11px] text-gray-500">
                    <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1">
                      ...
                      <span className="hidden sm:inline text-gray-400">·</span>
                      <span>Refactors stay under your review</span>
                    </div>
                    <p className="text-center text-gray-500">
                      Next: connect a repo and see your first safe refactor plan
                      in under 5 minutes.
                    </p>
                  </div>
                </form>
